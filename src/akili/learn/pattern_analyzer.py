...
- Manufacturer-specific biases (e.g. misreads thermal resistance in certain datasheets)
- Parameter-specific errors (e.g. max current off by 10x due to unit confusion)
...
class PatternAnalyzer:
    ...
    def get_auto_correction_rules(self) -> list[AutoCorrectionRule]:
        ...
        for p in patterns:
            if p.occurrences >= _AUTO_CORRECT_CONFIDENCE_THRESHOLD and p.auto_correctable:
                desc = (
                    f"When original matches '{p.original_pattern}', "
                    f"correct to '{p.corrected_pattern}'"
                )
                rules.append(
                    AutoCorrectionRule(
                        rule_id=f"auto_{p.pattern_id}",
                        pattern=p,
                        match_fn_description=desc,
                    )
                )
        return rules
        ...
        for rule in rules:
            ...
            if pat.category == "value_scaling" and canonical_type in pat.canonical_types:
                if _is_scaling_match(original_value, pat.original_pattern):
                    return _apply_scaling_correction(
                        original_value, pat.original_pattern, pat.corrected_pattern
                    )
        return None
    ...
    def _analyze_unit_confusion(self, corrections: list[Correction]) -> list[CorrectionPattern]:
        ...
        for swap, corrs in unit_swaps.items():
            ...
            desc = f"Unit confusion: {orig_u} -> {corr_u} ({len(corrs)} times)"
            patterns.append(
                CorrectionPattern(
        ...
        for factor, corrs in scale_factors.items():
            patterns.append(
                CorrectionPattern(
                    ...
                    corrected_pattern=f"divide_by_{factor}"
                    if "x" in factor and not factor.startswith("0.")
    ...
    def _analyze_by_canonical_type(self, corrections: list[Correction]) -> list[CorrectionPattern]:
        ...
        for ctype, count in by_type.items():
            ...
            if rate > 0.3 and count >= 3:
                desc = f"High correction rate for {ctype}: {count}/{total} ({rate:.0%})"
                patterns.append(
                    CorrectionPattern(
