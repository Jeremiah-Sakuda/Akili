...
class TestDetectMergeCandidates:
    ...
    def test_non_consecutive_pages_no_merge(self) -> None:
        ga = _make_grid("g1", page=0, rows=5, cols=3, origin_y=0.6, bbox_y1=0.6, bbox_y2=0.95)
        gb = _make_grid("g2", page=3, rows=3, cols=3, origin_y=0.05, bbox_y1=0.05, bbox_y2=0.20)
        candidates = detect_merge_candidates([ga, gb])
        assert len(candidates) == 0

    def test_different_column_count_no_merge(self) -> None:
        ga = _make_grid("g1", page=0, rows=5, cols=3, origin_y=0.6, bbox_y1=0.6, bbox_y2=0.95)
        gb = _make_grid("g2", page=1, rows=3, cols=5, origin_y=0.05, bbox_y1=0.05, bbox_y2=0.20)
        candidates = detect_merge_candidates([ga, gb])
        assert len(candidates) == 0

    def test_grid_not_near_bottom(self) -> None:
        ga = _make_grid("g1", page=0, rows=5, cols=3, origin_y=0.2, bbox_y1=0.2, bbox_y2=0.5)
        gb = _make_grid("g2", page=1, rows=3, cols=3, origin_y=0.05, bbox_y1=0.05, bbox_y2=0.20)
        candidates = detect_merge_candidates([ga, gb])
        assert len(candidates) == 0

    def test_different_docs_no_merge(self) -> None:
        ga = _make_grid(
            "g1", page=0, rows=5, cols=3, origin_y=0.6, doc_id="d1", bbox_y1=0.6, bbox_y2=0.95
        )
        gb = _make_grid(
            "g2", page=1, rows=3, cols=3, origin_y=0.05, doc_id="d2", bbox_y1=0.05, bbox_y2=0.20
        )
        candidates = detect_merge_candidates([ga, gb])
        assert len(candidates) == 0
...
class TestMergeGrids:
    def test_merge_with_matching_headers(self) -> None:
        headers = ["Param", "Min", "Max"]
        ga = _make_grid(
            "g1", page=0, rows=3, cols=3, origin_y=0.6, headers=headers, bbox_y1=0.6, bbox_y2=0.95
        )
        gb = _make_grid(
            "g2", page=1, rows=3, cols=3, origin_y=0.05, headers=headers, bbox_y1=0.05, bbox_y2=0.20
        )
    ...
    def test_merged_bbox(self) -> None:
        headers = ["A", "B"]
        ga = _make_grid(
            "g1", page=0, rows=3, cols=2, origin_y=0.6, headers=headers, bbox_y1=0.6, bbox_y2=0.95
        )
        gb = _make_grid(
            "g2", page=1, rows=2, cols=2, origin_y=0.05, headers=headers, bbox_y1=0.05, bbox_y2=0.20
        )
        cand = MergeCandidate(grid_a=ga, grid_b=gb, column_similarity=1.0)
        merged = merge_grids(cand)
...
class TestMergeMultipageTables:
    def test_integration(self) -> None:
        headers = ["Param", "Min", "Max"]
        ga = _make_grid(
            "g1", page=0, rows=5, cols=3, origin_y=0.6, headers=headers, bbox_y1=0.6, bbox_y2=0.95
        )
        gb = _make_grid(
            "g2", page=1, rows=3, cols=3, origin_y=0.05, headers=headers, bbox_y1=0.05, bbox_y2=0.20
        )
